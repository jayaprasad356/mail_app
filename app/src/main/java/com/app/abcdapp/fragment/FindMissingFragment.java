...
public class FindMissingFragment extends Fragment {
    ...
    ImageView ivInfo;
    TextView tvRegularTask,tvHightlight,tvLevelhint;
    RelativeLayout rl1 ;
    LinearLayout l1;
    CardView card1;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
       ...
       // AdId = session.getData(Constant.AD_REWARD_ID);
        ...
        tvHightlight.startAnimation(blink1);
...
//        if (session.getData(Constant.WITHDRAWAL).equals("0")) {
...
//                rvNamesData.setLayoutManager(linearLayoutManager);
                ...
                CustomLinearLayoutManager layoutManager = new CustomLinearLayoutManager(activity);
                layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                layoutManager.setReverseLayout(false);
                rvNamesData.setLayoutManager(layoutManager);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    rvNamesData.setImportantForAccessibility(View.IMPORTANT_FOR_ACCESSIBILITY_NO);
                }
...
//                rvCityData.setLayoutManager(linearLayoutManager);
                ...
                layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                layoutManager.setReverseLayout(false);
                rvCityData.setLayoutManager(layoutManager);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    rvCityData.setImportantForAccessibility(View.IMPORTANT_FOR_ACCESSIBILITY_NO);
                }
...
//                rvPincodeData.setLayoutManager(linearLayoutManager);
                ...
                layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                layoutManager.setReverseLayout(false);
                rvPincodeData.setLayoutManager(layoutManager);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    rvPincodeData.setImportantForAccessibility(View.IMPORTANT_FOR_ACCESSIBILITY_NO);
                }
...
//                startActivity(intent);
            ...
            public void onClick(View view) {
                ...
                if (ApiConfig.isConnected(activity)) {
                    if (btnsyncNow.isEnabled()) {
                        ...
                        if (session.getData(STATUS).equals("1")) {
                            if (ApiConfig.isConnected(activity)) {
                                if (session.getData(Constant.CODE_GENERATE).equals("1")) {
                                    ...
                                    bundle.putString(TASK_TYPE, TASK_TYPE);
    ...
    private void trynow() {
        ...
        if (session.getData(Constant.STATUS).equals("1") && !session.getData(Constant.LEVEL).equals("5") && session.getBoolean(Constant.L_TRIAL_AVAILABLE) && !session.getBoolean(Constant.L_TRIAL_STATUS)){
            tvLevelhint.setText("Click Here\nTry Level "+trial_level);
            tvLevelhint.setVisibility(View.VISIBLE);
            tvLevelhint.setOnClickListener(view -> {
